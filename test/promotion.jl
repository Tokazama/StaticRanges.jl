        ...
        for (mr2,br2,sr2) in ((OneToMRange(10),         OneTo(10),              OneToSRange(10)),
            ...
            if br != br2
                @testset "$(typeof(br)) & $(typeof(br2))" begin
                    br3 = promote_rule(typeof(br), typeof(br2))
                    @test eltype(promote_rule(typeof(mr), typeof(mr2))) == eltype(br3)
                    @test eltype(promote_rule(typeof(mr2), typeof(mr))) == eltype(br3)

                    @test eltype(promote_rule(typeof(sr), typeof(sr2))) == eltype(br3)
                    @test eltype(promote_rule(typeof(sr2), typeof(sr))) == eltype(br3)
                end
            end
        end
    end
