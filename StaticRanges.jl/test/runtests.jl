...
@testset "ranges" begin
    ...
    @testset "colon" begin
        #@inferred((:)(10, 1, 0))
        @inferred(srange(SVal{10}(), step=SVal{1}(), stop=SVal{0}()))

        #@inferred((:)(1, .2, 2))
        @inferred(srange(SVal{1}(), step=SVal{.2}(), stop=SVal{2}()))

        #@inferred((:)(1., .2, 2.))
        @inferred(srange(SVal(1.), step=SVal(.2), stop=SVal(2.)))

        #@inferred((:)(2, -.2, 1))
        @inferred(srange(SVal(2), step=SVal(-.2), stop=SVal(1)))

        #@inferred((:)(0.0, -0.5))
        @inferred(srange(SVal(0.0), SVal(-0.5)))

        #@inferred((:)(1, 0))
        @inferred(srange(SVal(1),SVal(0)))
    end

    @testset "indexing" begin
        L32 = @inferred(srange(SVal(Int32(1)), stop=SVal(Int32(4)), length=SVal(4)))
        ...
        @test @inferred(srange(Val(1), stop=Val(5))[1:4]) === srange(1, stop=4)
        @test @inferred(srange(Val(1.0), stop=Val(5))[1:4]) === srange(1.0, stop=4)

        @test srange(2, stop=6)[1:4] == srange(2:5)
        @test srange(1, stop=6)[2:5] === srange(2:5)
        @test srange(1, stop=6)[2:2:5] === srange(2:2:4)
        @test srange(1, step=2, stop=13)[2:6] === srange(3:2:11)
    ...
    @testset "length" begin
        ...
        #= TODO
        @test length(srange(Char(0):Char(0x001fffff))) == 2097152
        =#
