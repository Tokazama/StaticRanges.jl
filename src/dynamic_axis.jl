...
mutable struct DynamicAxis <: AbstractUnitRange{Int}
    ...
    DynamicAxis(stop) where {T<:Integer} = new(max(zero(T), stop))
    ...
    DynamicAxis(stop::T) where {T<:Integer} = DynamicAxis(Int(stop))

    DynamicAxis(r::AbstractRange{T}) where {T<:Integer} = DynamicAxis(r)
end
...
ArrayInterface.known_first(::Type{DynamicAxis}) = first(R)
ArrayInterface.known_step(::Type{DynamicAxis}) = one(T)
...
Base.promote_rule(::Type{DynamicAxis}, ::Type{OneTo{T}}) where {T} = OneTo{promote_type(T1,Int)}
Base.promote_rule(::Type{OneTo{T}}, ::Type{DynamicAxis}) where {T} = OneTo{promote_type(T1,Int)}
