...
values.
"""
index_by(a::AbstractAxis{name,K}, i::Function) where {name,K} = ByKey

index_by(a::AbstractAxis{name,K}, i::K) where {name,K} = ByKey
index_by(a::AbstractAxis{name,K}, i::AbstractVector{K}) where {name,K} = ByKey

index_by(a::AbstractAxis{name,K}, i::K) where {name,K<:Integer} = ByKey
index_by(a::AbstractAxis{name,K}, i::AbstractVector{K}) where {name,K<:Integer} = ByKey

index_by(a::AbstractAxis{name,K}, i::I) where {name,K,I<:Integer} = ByValue
index_by(a::AbstractAxis{name,K}, i::AbstractVector{I}) where {name,K,I<:Integer} = ByValue

@inline @propagate_inbounds function Base.to_index(a::AbstractAxis, i)
...
@propagate_inbounds function _to_index(b::ByValueTrait, a, i::AbstractVector)
    ...
    return @inbounds __to_index(a, i, i)
end
